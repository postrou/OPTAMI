...
class PrimalDualAccelerated(Optimizer):
    def __init__(
        ...
        C = (
            p_order
            / 2
            * ((p_order + 1) / (p_order - 1) * (M_squared - M_p_squared)) ** 0.5
        )
        A_factor = (
            (p_order - 1)
            * (M_squared - M_p_squared)
            / (4 * (p_order + 1) * p_order**2 * M_squared)
        ) ** (p_order / 2)
    ...
    def step(self, closure=None):
        ...
        phi_next, grad_phi_next = self._calculate_closure_and_its_grad(
            closure, A, A_next, params
        )
        ...
        with torch.no_grad():
            for i, param in enumerate(params):
                ...
                if k == 0:
                    ...
                    X_hat_matrix_next = (
                        1 - A_over_A_next
                    ) * X_matrix + A_over_A_next * X_hat_matrix
